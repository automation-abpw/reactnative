...
import com.abpm.excelhandler.ReadExcel;
	...
	public class GetOtpVerified 
	
	{
	
		public static String otp;
		public static String otp()throws MalformedURLException
		
{
	
		 ReadExcel.Excel();	
		 String email = ReadExcel.VerEmail;	
		 System.out.println("-----GetOTP()------");
		 WebDriver driver = new FirefoxDriver();
		 driver.manage().window().maximize();
		 
		 driver.get("http://10.132.100.58/adminer.php?server=10.132.100.58&username=root&db=testdbnew_ee");
		
		 driver.findElement(By.id("username")).clear();
		 driver.findElement(By.id("username")).sendKeys("root");
		 driver.findElement(By.name("auth[password]")).clear();
		 driver.findElement(By.name("auth[password]")).sendKeys("admin");
